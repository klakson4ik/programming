...
class Base
{
	...
	public function __construct()
	{
		if (Config::has('sitemap')) {
			...
			$this->domain = $config['domain'] ?? env('APP_URL');
	...
	private function modelElementAdd(array &$result, mixed $element, array $settings, string $fieldUrl): void
	{
		$startUrl = $this->domain . '/' . ($settings['start-url'] ?? $element->getTable()) . '/';
	...
	{
		$class = 'App\\Services\\SitemapService\\StructureAction';
		if (isset($this->structure['method']) && method_exists($class, $this->structure['method'])) {
			$urls =  call_user_func([$class, $this->structure['method']]);
		} else {
			throw new Exception('Нет метода для форирования структуры');
		}

		if (isset($this->structure['exclude']) && !empty($this->structure['exclude']))
			$urls = $this->exlcudeUrls($urls);
		if (isset($this->structure['include']) && !empty($this->structure['include']))
			$urls = $this->addCustomUrls($urls);

		$result[] = $this->addCellValue($this->domain, 'daily', date('c', time()), '1');
	...
	private function exlcudeUrls(array $data): array
	{
		$result = array_filter($data, fn($item) => !in_array($item, $this->structure['exclude']));
		return [];
	}
