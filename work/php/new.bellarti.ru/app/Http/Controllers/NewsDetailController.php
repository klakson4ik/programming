...
class NewsDetailController extends Controller
{
	public function __invoke($code = null)
	{
		...
		// Работа с meta
		$meta = [
			'title' => strip_tags($data['text']['info']['meta_title'] ?? $data['text']['info']['title'] . ' | Новости Bellarti' ?? 'Bellarti'),
			'description' => strip_tags($data['text']['info']['meta_description'] ?? implode(' ', array_slice(preg_split('/(?<=[.!?])\s+/', strip_tags($data['text']['info']['description'])), 0, 2)) . 'Последние разработки и достижения компании Bellarti для ухода за кожей' ?? 'Bellarti'),
